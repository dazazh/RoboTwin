...
def main():
    ...
    point_cloud_arrays, point_cloud_from_sensor_arrays, episode_ends_arrays, action_arrays, state_arrays, joint_action_arrays = [], [], [], [], [], []
    
    while os.path.isdir(load_dir+f'/episode{current_ep}') and current_ep < num:
        print(f'processing episode: {current_ep + 1} / {num}', end='\r')
        file_num = 0
        point_cloud_sub_arrays = []
        point_cloud_from_sensor_sub_arrays = []
        state_sub_arrays = []
        action_sub_arrays = [] 
        ...
        while os.path.exists(load_dir+f'/episode{current_ep}'+f'/{file_num}.pkl'):
            ...
            pcd = data['pointcloud'][:,:]
            pcd_from_sensor = data['pointcloud_from_sensor'][:,:]
            action = data['endpose']
            ...
            point_cloud_from_sensor_sub_arrays.append(pcd_from_sensor)
        ...
        point_cloud_from_sensor_arrays.extend(point_cloud_from_sensor_sub_arrays)
    ...
    point_cloud_from_sensor_arrays = np.array(point_cloud_from_sensor_arrays)
    ...
    point_cloud_from_sensor_chunk_size = (100, point_cloud_from_sensor_arrays.shape[1], point_cloud_from_sensor_arrays.shape[2])
    print(point_cloud_chunk_size)
    print(point_cloud_from_sensor_chunk_size)
    zarr_data.create_dataset('point_cloud', data=point_cloud_arrays, chunks=point_cloud_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('point_cloud_from_sensor', data=point_cloud_from_sensor_arrays, chunks=point_cloud_from_sensor_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
