import sys
sys.path.append('./') 
sys.path.insert(0, './policy/Diffusion-Policy-DA(encoder)') 
...
def get_policy(checkpoint, output_dir, device):
    ...
    # load checkpoint
    payload = torch.load(open('./policy/Diffusion-Policy-DA(encoder)/'+checkpoint, 'rb'), pickle_module=dill)
    cfg = payload['cfg']
...
def test_policy(task_name, Demo_class, args, dp: DP, st_seed, test_num=20):
    ...
    while succ_seed < test_num:
        ...
        Demo_class.setup_demo(now_ep_num=now_id, seed = now_seed, is_test = True, ** args)
        Demo_class.apply_dp_da_encoder(dp, args)
...
def main(usr_args):
    ...
    save_dir = Path(f'eval_result/dp_da_encoder/{task_name}_{usr_args.head_camera_type}/{usr_args.expert_data_num}')
