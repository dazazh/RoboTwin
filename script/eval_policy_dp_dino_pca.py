import sys
sys.path.append('./') 
sys.path.insert(0, './policy/Diffusion-Policy-Dino-PCA') 
...
def get_policy(checkpoint, output_dir, device):
    ...
    # load checkpoint
    payload = torch.load(open('./policy/Diffusion-Policy-Dino-PCA/'+checkpoint, 'rb'), pickle_module=dill)
    cfg = payload['cfg']
...
def main(usr_args):
    ...
    save_dir = Path(f'eval_result/dp_dino_pca/{task_name}_{usr_args.head_camera_type}/{usr_args.expert_data_num}')
