...
import json
import traceback
from video_generate import images_to_video
...
def run(Demo_class, args):
    ...
    if args['collect_data']:
        ...
        for id in range(args['st_episode'], args['episode_num']):
            ...
            Demo_class.close()
            print('\nsuccess!')
            # images_to_video('./data/'+args['task_name']+'_'+str(args['head_camera_type'])+f'/episode{id}/camera/color/head',
            #                 './data/'+args['task_name']+'_'+str(args['head_camera_type'])+f'/episode{id}/camera/head.mp4', 
            #                 15)
