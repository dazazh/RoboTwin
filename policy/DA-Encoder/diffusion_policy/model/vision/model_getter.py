...
class DPTHead(nn.Module):
    ...
    def forward(self, out_features, patch_h, patch_w):
        ...
        features = self.scratch.refinenet1(layer_1_rn)
...
class DPT_DINOv2(nn.Module):
    ...
    def forward(self, x):
        ...
        features = self.pretrained.get_intermediate_layers(x, 1, return_class_token=True)
        
        patch_h, patch_w = h // 14, w // 14

        features = self.depth_head(features, patch_h, patch_w)
        features = self.pooling(features)
        out_features = features.flatten(start_dim=1)
        return out_features
...
def get_resnet_da(name, weights=None, **kwargs):
    ...
    da_encoder = DepthAnything.from_pretrained('LiheYoung/depth_anything_vits14')
